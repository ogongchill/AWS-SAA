...
```gpt
> E. Lake Formation 청사진 + Glue + Parquet + S3

요구사항 중 일부를 다시 보면:

- 배치 데이터 + 실시간 스트림 데이터 → 한 곳에 통합  
- 수신 데이터를 처리한 다음 다른 S3 버킷에 준비
- 이후 BI 도구에서 KPI 시각화

E는 이걸 거의 템플릿처럼 그대로 해 줘:

- Lake Formation 청사진
    - 어떤 데이터를 데이터 레이크(S3)에 모을지 정의
    - 데이터 소스에 대한 접근·보안·카탈로그를 자동으로 구성
- AWS Glue 크롤러 & ETL 작업 
    - 다양한 소스(DB, 로그, 스트림 적재된 S3 등)를 크롤링하고 스키마 자동 인식
    - ETL 작업으로 데이터를 **정제/변환 → Apache Parquet 형식**으로 저장
- S3에 저장 (데이터 레이크) 
    - Parquet는 컬럼형 포맷이라 **Athena, Redshift Spectrum, EMR** 등에서 쿼리 비용과 성능에 유리

즉, **데이터 통합 + 처리 + S3에 준비**라는 요구사항을 가장 적은 운영 오버헤드로 만족시키는 정석 조합이야.
```
